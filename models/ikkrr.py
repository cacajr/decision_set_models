...
class IKKRR:
    ...
    def __create_wcnf_formula(self, previous_solution, X_norm, y):
        ...
        if len(previous_solution) == 0:
            ...
            # (19) and (20)
            p_literals = self.__get_p_literals(normal_features)
        ...
        for l in range(self.__max_rule_set_size):  # l ∈ {1, ..., k}
            for j in range(len(normal_features)):  # j ∈ {1, ..., m}
                wcnf_formula.append([self.__p(j,l), self.__pl(j,l)])
        ...
        for i in np.where(y == 0)[0]:   # Xi ∈ E-
            for l in range(self.__max_rule_set_size):   # l ∈ {1, ..., k}
                ...
                for j in range(len(normal_features)):   # j ∈ {1, ..., m}
                    if X_norm[i][j] == 0:
                        clause.append(-self.__p(j,l))
                    else:
                        clause.append(-self.__pl(j,l))
        ...
        for i in np.where(y == 1)[0]:   # Xi ∈ E+
            for l in range(self.__max_rule_set_size):  # l ∈ {1, ..., k}
                for j in range(len(normal_features)):  # j ∈ {1, ..., m}
                    if X_norm[i][j] == 0:
                        wcnf_formula.append([self.__p(j,l), -self.__cr(l, i)])
                    else:
                        wcnf_formula.append([self.__pl(j,l), -self.__cr(l, i)])
    ...
    def __p(self, j, l):
        return self.__literals.id(f'p{j}{l}')
    ...
    def __get_p_literals(self, features):
